  ...
  <div class="popup">
    <div class="popup__content">
      ...
      <div class="resetPassword-form">
        <input v-model="registerEmail" type="text" placeholder="電子郵件" @blur="validateEmail(registerEmail)" @input="clearError" :class="{ 'error': registerEmailError }"/>
        <span v-if="registerEmailError" class="error">{{ registerEmailError }}</span>
      </div>
      <div class="form--btn">
        <button @click="resetPassword">密碼重設</button>
      </div>
...
import { fetchForgetPassword } from '@/api/index';
...
const validateEmail = (email) => {
  ...
  registerEmailError.value = email.length > 0 && emailPattern.test(email) ? '' : '無效的電子郵件地址';
};
...
const resetPassword = () => {
  if (!registerEmailError.value) {
    fetchForgetPassword({'email': registerEmail.value})
    .then(res => {
